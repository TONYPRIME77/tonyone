...
API_PROVIDERS = {
    "openrouter": {
        ...
        "free":[
            ...
            ("meta-llama/llama-3.3-70b-instruct:free", "Llama 3.3 70B   — large & powerful     [FREE]"),
...
def query_ai(prompt, cfg, history, retry=True):
    ...
    try:
        ...
        # Auto-retry with free fallback on credit/quota errors
        if retry and e.code in (402, 429) and provider == "openrouter":
            fallback = API_PROVIDERS["openrouter"]["default_free"]
            if model != fallback:
                cfg_tmp = dict(cfg); cfg_tmp["model"] = fallback
                return query_ai(prompt, cfg_tmp, history, retry=False)
        return "", err_msg
...
def handle_error(error, cfg):
    ...
    if "403" in error or "1010" in error or "cloudflare" in e or "ray id" in e:
        ...
        _wm_say("Model not found. Run 'apis' to browse available models.",col=C.YELLOW,delay=SPEED_FAST)
