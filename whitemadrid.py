...
#  BOOT
# ══════════════════════════════════════════════════════
# ── Hacker animation helpers ──────────────────────────────────────────
# ══════════════════════════════════════════════════════
#  ANIMATION ENGINE
# ══════════════════════════════════════════════════════

# Typing speeds (seconds per char)
SPEED_FAST   = 0.010
SPEED_NORMAL = 0.022
SPEED_SLOW   = 0.040

def _glitch_char():
    return random.choice("@#$%&?!<>[]{}|/*^~`01ABCDEFabcdef")

def _type(text, delay=SPEED_NORMAL, col=C.GREEN, newline=True, indent="  "):
    """Core typing animation — smooth with natural variance."""
    sys.stdout.write(indent)
    for i, ch in enumerate(text):
        sys.stdout.write(c(col, ch))
        sys.stdout.flush()
        # Natural pauses: longer after punctuation, shorter mid-word
        if ch in (".", "!", "?"):
            time.sleep(delay * 4)
        elif ch in (",", ";", ":"):
            time.sleep(delay * 2)
            ...
            time.sleep(delay + random.uniform(-delay*0.3, delay*0.4))
    if newline:
        sys.stdout.write("\n")
        sys.stdout.flush()

def _type_lines(lines, delay=SPEED_NORMAL, col=C.GREEN, gap=0.04):
    """Type multiple lines with a gap between each."""
    for line in lines:
        _type(line, delay=delay, col=col)
        time.sleep(gap)

def _wm_say(text, col=C.CYAN, delay=SPEED_NORMAL, tag="[WM]"):
    """WHITE MADRID speaks — typed with tag prefix."""
    ...
    for ch in text:
        ...
        else:                       time.sleep(delay + random.uniform(-0.005, 0.012))
    sys.stdout.write("\n")
    sys.stdout.flush()

def _glitch_text(text, col=C.GREEN, cycles=8, delay=0.045, indent="  "):
    """Text glitches with random chars then resolves cleanly."""
    for i in range(cycles):
        ratio = 0.6 * (1 - i / cycles)   # glitch fades over cycles
        glitched = ""
        for ch in text:
            if ch != " " and random.random() < ratio:
                glitched += _glitch_char()
            else:
                glitched += ch
        sys.stdout.write("\r" + indent + c(col, glitched) + "   ")
        sys.stdout.flush()
        time.sleep(delay)
    # Final clean render
...
def _progress_bar(label, width=38, col=C.GREEN, delay=0.025):
    """Smooth progress bar with variable fill speed."""
    sys.stdout.write(f"  {c(C.GRAY, label)} {c(C.GRAY, '[')} ")
    sys.stdout.flush()
    for i in range(width):
        spd = delay * random.uniform(0.3, 2.2)
        time.sleep(spd)
        ch = "█" if random.random() > 0.15 else random.choice(["▓","▒","░"])
        sys.stdout.write(c(col, ch))
        sys.stdout.flush()
    sys.stdout.write(f" {c(C.GRAY,']')} {c(col+C.BOLD, 'DONE')}\n")
    sys.stdout.flush()

def _matrix_rain(rows=5, cols=None, delay=0.035):
    """Matrix-style falling characters."""
    if cols is None:
        cols = min(twidth() - 4, 80)
    chars = "01アイウエオカキクケコサシスセソタチツテトナニヌネノ@#$%&?!<>{}[]|"
    streams = [random.randint(0, len(chars)-1) for _ in range(cols)]
    bright  = [random.random() > 0.85 for _ in range(cols)]
    for row in range(rows):
        line = ""
        for j in range(cols):
            ch = chars[streams[j] % len(chars)]
            streams[j] += random.randint(1, 4)
            bright[j] = random.random() > 0.88
            col_c = (C.WHITE + C.BOLD) if bright[j] else C.GREEN
            line += c(col_c, ch)
        sys.stdout.write("  " + line + "\n")
        sys.stdout.flush()
        time.sleep(delay + random.uniform(-0.01, 0.02))

def _scan_line(text, col=C.CYAN, delay=0.018):
    """Scan-line effect: text sweeps in left to right with cursor."""
    w = twidth() - 4
    for i in range(len(text) + 1):
        chunk  = text[:i]
        rest   = " " * max(0, w - len(text))
        cursor = c(C.WHITE+C.BOLD, "█") if i < len(text) else " "
        sys.stdout.write("\r  " + c(col+C.BOLD, chunk) + cursor + rest)
        sys.stdout.flush()
        time.sleep(delay + random.uniform(-0.005, 0.01))
    sys.stdout.write("\n")
    sys.stdout.flush()

def _hex_dump_line(delay=0.030):
    """One line of fake hex dump output."""
    ...
    line = (f"  {c(C.GRAY, hex(addr)[2:].upper().zfill(6))}  "
            f"{c(C.GREEN, hexb)}  {c(C.CYAN, asc)}")
    sys.stdout.write(line + "\n")
    sys.stdout.flush()
    time.sleep(delay)

def _fake_scroll(lines_data, delay=0.055):
    """Scroll lines like a boot log."""
    for line in lines_data:
        sys.stdout.write("  " + line + "\n")
        sys.stdout.flush()
        time.sleep(delay + random.uniform(0, 0.035))
...
def _get_greeting(operator="Operator"):
    ...
    if 5 <= h < 12:
        ...
        lines = [
            ...
            f"Hey {operator}. Mid-day recon session? I'm ready.",
        ]
    elif 17 <= h < 21:
        period = "evening"
        lines = [
            ...
            f"Evening session, {operator}? The network is quieter now. Perfect.",
        ]
    else:
        period = "night"
        lines = [
            ...
            f"Night ops, {operator}? Good. Less traffic, more opportunity.",
        ]
    return random.choice(lines), period
...
HACKER_JOKES = [
    ...
    "My password is 'incorrect'. So whenever I forget it, it tells me: Your password is incorrect.",
    "A wifi password is just a socially acceptable way to say: I don't trust you yet.",
    ...
    "Tried to catch some fog earlier. I mist. Just like that firewall.",
    "The best antivirus? Not clicking that link your uncle sent.",
    "A TCP packet walks into a bar. The bartender says: 'You want a drink?' TCP replies: 'Yes. Did you get that? You got that right?'",
...
# ══════════════════════════════════════════════════════
#  MINI GAMES & IDLE ACTIVITIES
# ══════════════════════════════════════════════════════
def _play_trivia():
    """Random CTF/security trivia question."""
    ...
    try:
        guess = input(c(C.GREEN, "  A: ")).strip()
    except (KeyboardInterrupt, EOFError):
        print(); return
    if guess.lower() in a.lower() or a.lower() in guess.lower():
        _wm_say(f"Correct! {a}. You know your stuff.", col=C.GREEN, delay=SPEED_FAST)
    else:
        _wm_say(f"Not quite. Answer: {c(C.WHITE, a)}. Study up.", col=C.YELLOW, delay=SPEED_FAST)
    print()

def _play_binary():
    """Convert decimal to binary quiz."""
    num  = random.randint(1, 255)
    ans  = bin(num)[2:]
    print()
    _wm_say(f"Binary quiz — convert {c(C.WHITE+C.BOLD, str(num))} to binary:", col=C.CYAN, delay=SPEED_NORMAL)
    try:
        guess = input(c(C.GREEN, "  Binary: ")).strip().lstrip("0b")
    except (KeyboardInterrupt, EOFError):
        print(); return
    if guess == ans:
        _wm_say(f"Perfect. {num} = {c(C.GREEN, ans)}. Clean.", col=C.GREEN, delay=SPEED_FAST)
    else:
        _wm_say(f"Nope. {num} = {c(C.WHITE, ans)}. Binary is fundamental, {c(C.CYAN, 'study it')}.", col=C.YELLOW, delay=SPEED_FAST)
    print()

def _play_hexquiz():
    """Convert decimal to hex quiz."""
    num = random.randint(1, 255)
    ans = hex(num)[2:].upper()
    print()
    _wm_say(f"Hex quiz — convert {c(C.WHITE+C.BOLD, str(num))} to hexadecimal:", col=C.CYAN, delay=SPEED_NORMAL)
    try:
        guess = input(c(C.GREEN, "  0x")).strip().upper().lstrip("0X")
    except (KeyboardInterrupt, EOFError):
        print(); return
    if guess == ans:
        _wm_say(f"Spot on. {num} = 0x{c(C.GREEN, ans)}.", col=C.GREEN, delay=SPEED_FAST)
    else:
        _wm_say(f"Wrong. {num} = 0x{c(C.WHITE, ans)}. Get familiar with hex — you'll need it.", col=C.YELLOW, delay=SPEED_FAST)
    print()

def _tell_joke():
    """Hacker joke with typing animation."""
    print()
...
def _share_wisdom():
    """Hacker wisdom quote."""
    print()
    _wm_say(random.choice(HACKER_WISDOM), col=C.MAGENTA, delay=SPEED_SLOW, tag="[WM]")
    print()

def relax_mode(operator="Operator"):
    """Idle/relax mode — games, jokes, wisdom."""
    print()
    _wm_say(f"Switching to relax mode, {operator}. Taking a breather.", col=C.CYAN)
    time.sleep(0.3)
    _wm_say("I've got trivia, jokes, binary/hex quizzes, and wisdom. Your call.", col=C.GRAY, delay=SPEED_FAST)
    print()
    options = [
    ...
    while True:
        try:
            choice = input(c(C.GREEN+C.BOLD, "  relax> ")).strip()
        except (KeyboardInterrupt, EOFError):
            print(); break
        if choice == "0" or choice.lower() in ("exit","back","q"):
            _wm_say("Back to ops. Let's go.", col=C.GREEN, delay=SPEED_FAST)
            break
        elif choice == "1": _play_trivia()
        elif choice == "2": _play_binary()
        elif choice == "3": _play_hexquiz()
        elif choice == "4": _tell_joke()
        elif choice == "5": _share_wisdom()
        elif choice == "6":
            act = random.choice([_play_trivia, _play_binary, _play_hexquiz, _tell_joke, _share_wisdom])
            act()
        else:
            _wm_say("Enter 0-6. Simple.", col=C.GRAY, delay=SPEED_FAST)
...
def boot_sequence(cfg):
    ...
    print()
    _matrix_rain(rows=4, delay=0.03)
    time.sleep(0.1)

    # Phase 2 — Banner with glitch effect
    ...
    _glitch_text("  WHITE MADRID  v5.0  —  JARVIS EDITION", col=C.GREEN, cycles=8, delay=0.04)
    _glitch_text(f"  DEVELOPER: {DEVELOPER}  |  ETHICAL HACKING AI", col=C.CYAN, cycles=5, delay=0.035)
    print()

    # Phase 4 — Fake hex dump (looks like memory scan)
    sys.stdout.write(c(C.GRAY, "  [*] Scanning memory map...\n"))
    sys.stdout.flush()
    time.sleep(0.1)
    for _ in range(5):
        _hex_dump_line(delay=0.04)

    print()

    # Phase 5 — System scan lines
    scan_items = [
        f"SYSTEM      {si.get('host','device').upper()}",
        f"KERNEL      {si.get('kernel','unknown')}",
        f"ARCH        {si.get('arch','?').upper()}",
        f"PLATFORM    {'TERMUX/ANDROID' if is_termux() else 'LINUX/MACOS'}",
        f"PROVIDER    {API_PROVIDERS.get(prov,{}).get('name',prov).upper()}",
        f"MODEL       {model[:42].upper()}",
    ...
    for item in scan_items:
        ...
        sys.stdout.write(f"  {c(C.GRAY, '│')} {c(C.GRAY, key.strip()):<12}  {c(C.GREEN, val.strip())}\n")
        sys.stdout.flush()
        time.sleep(0.06)
    print()
    ...
    boot_tasks = [
        ("AI CORE      ", C.CYAN,    40, 0.018),
        ("API REGISTRY ", C.CYAN,    40, 0.015),
        ("TOOL MODULES ", C.CYAN,    40, 0.016),
        ("JARVIS ENGINE", C.MAGENTA, 40, 0.014),
        ("ENCRYPTION   ", C.GREEN,   40, 0.012),
    ]
    for label, col, width, delay in boot_tasks:
        _progress_bar(label, width=width, col=col, delay=delay)
        time.sleep(0.05)
    print()

    # Phase 7 — Boot log scroll
    boot_log = [
        c(C.GRAY,  "[  0.001] Kernel boot OK"),
        c(C.GRAY,  "[  0.012] Loading AI inference engine..."),
        c(C.GREEN,  "[  0.089] Claude interface mounted"),
        c(C.GRAY,  "[  0.134] OpenRouter gateway online"),
        c(C.GRAY,  "[  0.201] Tool catalogue loaded (30 entries)"),
        c(C.GREEN,  "[  0.298] JARVIS personality module active"),
        c(C.YELLOW, "[  0.312] Ethical constraints enforced"),
        c(C.GRAY,  "[  0.401] Session encryption enabled"),
        c(C.GREEN,  "[  0.500] WHITE MADRID v5.0 ready"),
    ]
    _fake_scroll(boot_log, delay=0.06)
    print()

    # Phase 8 — Final status bar
    sep = "═" * (w - 4)
    ...
    if sessions == 0:
        _scan_line(f"  FIRST BOOT — Welcome, {operator.upper()}. All systems online.", col=C.GREEN, delay=0.018)
    else:
        _scan_line(f"  SESSION #{sessions+1}  |  {date_str()}  {now_str()}  |  {period.upper()}", col=C.GREEN, delay=0.016)

    _type(
        f"[AUTH] {operator} | {API_PROVIDERS.get(prov,{}).get('name',prov)} | {model[:30]}",
        delay=SPEED_FAST, col=C.GRAY, indent="  "
    )
    sys.stdout.write(c(C.GREEN, "  " + sep) + "\n")
    sys.stdout.flush()
    print()

    # Time-based greeting typed out
    _wm_say(greeting, col=C.CYAN, delay=SPEED_NORMAL)
    time.sleep(0.15)
    _wm_say("Commands: help · apis · topics · relax · install", col=C.GRAY, delay=SPEED_FAST)
...
def query_ai(prompt, cfg, history):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=90) as r:
            raw = r.read()
            # decompress if server sent gzip-compressed response
        ...
        except: return "", f"HTTP {e.code}: {raw_err[:200]}"
...
def print_help():
    ...
    secs=[
        ...
        ("RELAX / IDLE",[
            ...
            ("trivia",             "Random security trivia question"),
        ]),
        ("SHELL",[
...
def main():
    ...
    while True:
        ...
        elif cmd in ("relax","chill","break","idle","game","games","fun"):
            relax_mode(cfg.get("operator","Operator")); continue
        elif cmd in ("joke",):      _tell_joke(); continue
        elif cmd in ("wisdom",):    _share_wisdom(); continue
        elif cmd in ("trivia",):    _play_trivia(); continue
