...
def _glitch_char():
    glitch="@#$%&?!<>[]{}|\\/*^~`01"
    return random.choice(glitch)

def _type_line(text, delay=0.03, col=C.GREEN, newline=True):
    for ch in text:
...
def _glitch_text(text, col=C.GREEN, cycles=6, delay=0.05):
    for i in range(cycles):
        ...
        for ch in text:
            if ch != " " and random.random() < 0.4:
                ...
                glitched += ch
        sys.stdout.write("\r  " + c(col, glitched) + "  ")
        sys.stdout.flush()
        time.sleep(delay)
    sys.stdout.write("\r  " + c(col, text) + "  \n")
    sys.stdout.flush()

def _progress_bar(label, width=40, col=C.GREEN, delay=0.03):
    sys.stdout.write(f"  {c(C.GRAY, label)} [")
    sys.stdout.flush()
    for i in range(width):
        time.sleep(delay * random.uniform(0.5, 1.5))
        ch = random.choice(["█","▓","▒"]) if i < width-1 else "█"
        sys.stdout.write(c(col, ch))
        sys.stdout.flush()
    sys.stdout.write(f"] {c(C.GREEN, '100%')}\n")
    sys.stdout.flush()

def _matrix_rain(rows=6, cols=None, delay=0.04):
    if cols is None:
        cols = min(twidth() - 4, 78)
    ...
    for _ in range(rows):
        ...
        for j in range(cols):
            ...
            streams[j] += random.randint(1, 3)
            bright = random.random() > 0.85
            col_c = C.WHITE + C.BOLD if bright else C.GREEN
            line += c(col_c, ch)
        sys.stdout.write("  " + line + "\n")
        sys.stdout.flush()
        time.sleep(delay)

def _scan_line(text, col=C.CYAN, delay=0.002):
    w = twidth() - 4
    for i in range(len(text)+1):
        chunk   = text[:i]
        rest    = " " * (w - i)
        cursor  = c(C.WHITE+C.BOLD, "█") if i < len(text) else ""
        sys.stdout.write("\r  " + c(col, chunk) + cursor + rest)
        sys.stdout.flush()
        time.sleep(delay)
    sys.stdout.write("\n")
    sys.stdout.flush()

def _hex_dump_line(delay=0.03):
    addr = random.randint(0x00000000, 0xFFFFFF00)
    ...
    line = f"  {c(C.GRAY, hex(addr)[2:].upper().zfill(8))}  {c(C.GREEN, hexb)}  {c(C.CYAN, asc)}"
    sys.stdout.write(line + "\n")
    sys.stdout.flush()
    time.sleep(delay)

def _fake_scroll(lines_data, delay=0.05):
...
def boot_sequence(cfg):
    ...
    # Phase 8 — Final status bar with typing effect
    sep = "═" * (w - 4)
    ...
    if sessions == 0:
        msg = f"  FIRST BOOT — Welcome, {cfg.get('operator','Operator').upper()}. All systems online."
    else:
        msg = f"  {random.choice(BOOT_LINES).upper()}  SESSION #{sessions+1}"

    _scan_line(msg, col=C.GREEN, delay=0.015)

    _type_line(
        f"  [AUTH] Operator: {cfg.get('operator','Operator')} | Provider: "
        f"{API_PROVIDERS.get(prov,{}).get('name',prov)} | {date_str()} {now_str()}",
        delay=0.012, col=C.GRAY
    )
    sys.stdout.write(c(C.GREEN, "  " + sep) + "\n")
    sys.stdout.flush()
    print()

    _type_line("  [WM] Type your query or command. Try: help · apis · topics", delay=0.018, col=C.CYAN)
    _type_line("  [WM] Authorized security research only.", delay=0.018, col=C.YELLOW)
    print()
