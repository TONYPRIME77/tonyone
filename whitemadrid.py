...
def main():
    ...
    while True:
        ...
        if error:
            print(c(C.RED,f"\n  [WM] Error: {error}"))
            if "key" in error.lower() or "auth" in error.lower() or "401" in error:
                prov2=cfg.get("provider",DEFAULT_PROVIDER)
                print(c(C.YELLOW,f"  [WM] Try: setkey {prov2}   or   provider groq  (free, no card)"))
            elif "credits" in error.lower() or "afford" in error.lower():
                print(c(C.YELLOW,"  [WM] Out of credits. Options:"))
